...
struct DepletionInsightsView: View {
    ...
    @State private var showingFeedbackItem: FreshliItem? = nil
    @State private var feedbackItem: FreshliItem? = nil
    ...
    private func feedbackSheetContent(for item: FreshliItem) -> some View {
        VStack(spacing: PSSpacing.lg) {
            VStack(spacing: PSSpacing.md) {
                HStack {
    ...
    private func markAsConsumed(_ item: FreshliItem) {
    ...
    private func markAsStillHave(_ item: FreshliItem) {
...
#Preview {
    DepletionInsightsView()
        .modelContainer(for: [FreshliItem.self, ConsumptionRecord.self], inMemory: true)
}
