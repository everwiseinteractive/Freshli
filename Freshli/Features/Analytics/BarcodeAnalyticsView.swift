    ...
    private var hero: some View {
        VStack(spacing: PSSpacing.md) {
            ...
            VStack(spacing: PSSpacing.xs) {
                Text(String(localized: "Barcode Analytics"))
                    .font(.system(size: PSLayout.scaledFont(20), weight: .black, design: .rounded))
                Text(String(localized: "EPR (Extended Producer Responsibility) cost of your packaging waste, tracked per barcode."))
    ...
    private var emptyState: some View {
        VStack(spacing: PSSpacing.md) {
            Image(systemName: "barcode")
                ...
                .foregroundStyle(PSColors.textTertiary)
            Text(String(localized: "No barcode data yet"))
                .font(.system(size: PSLayout.scaledFont(16), weight: .bold))
                .foregroundStyle(PSColors.textPrimary)
            Text(String(localized: "Scan barcodes when you add items to unlock EPR cost tracking and packaging waste insights."))
                .font(.system(size: PSLayout.scaledFont(13), weight: .medium))
                .foregroundStyle(PSColors.textSecondary)
                .multilineTextAlignment(.center)
        }
    ...
    private var footer: some View {
        HStack(spacing: PSSpacing.sm) {
            Image(systemName: "info.circle.fill")
                ...
                .foregroundStyle(PSColors.textTertiary)
            Text(String(localized: "EPR costs estimate the packaging levy producers pay. High scores mean switching brands saves money and waste."))
                .font(.system(size: PSLayout.scaledFont(11), weight: .medium))
                .foregroundStyle(PSColors.textTertiary)
                .lineSpacing(2)
        }
