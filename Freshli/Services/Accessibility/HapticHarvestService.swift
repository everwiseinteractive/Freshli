...
final class HapticHarvestService {
    ...
    // MARK: - Freshli Signature Patterns (delegated to FreshliHapticManager)

    /// Sharp snap when a fresh item is added — delegates to FreshliHapticManager.
    func crispSnap() {
        FreshliHapticManager.shared.crispSnap()
    }

    /// Low rumble when an item is marked Wasted — delegates to FreshliHapticManager.
    func softWilt() {
        FreshliHapticManager.shared.softWilt()
    }

    /// Dual-pulse heartbeat when a neighbor Likes a donation — delegates to FreshliHapticManager.
    func communityHeartbeat() {
        FreshliHapticManager.shared.communityHeartbeat()
    }
    ...
    func prepare() {
        prepareCoreHaptics()
        FreshliHapticManager.shared.prepare()
    }

    /// Stops the haptic engine (called on app background).
    func stop() {
        hapticEngine?.stop(completionHandler: nil)
        FreshliHapticManager.shared.stop()
