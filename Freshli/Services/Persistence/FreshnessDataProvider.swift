...
enum FreshnessDataProvider {
    private static let appGroupID = "group.everwise.interactive.Freshli"
    ...
    static func updateFreshnessData(modelContext: ModelContext) {
        ...
        do {
            ...
            let savedItemsDescriptor = FetchDescriptor<FreshliItem>(
            ...
            let expiredItemsDescriptor = FetchDescriptor<FreshliItem>(
                predicate: #Predicate { item in
                    !item.isConsumed &&
                    !item.isDonated &&
                    !item.isShared &&
    ...
    private static func calculateStreak(
        ...
        do {
            let yesterdayExpiredDescriptor = FetchDescriptor<FreshliItem>(
                predicate: #Predicate { item in
                    !item.isConsumed &&
                    !item.isDonated &&
                    !item.isShared &&
