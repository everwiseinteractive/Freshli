...
final class RescueChefService {
    ...
    func generateMissions(for items: [PantryItem]) {
    ...
    private func findAtRiskItems(in items: [PantryItem]) -> [PantryItem] {
    ...
    private func generateMissionsFromItems(_ items: [PantryItem]) -> [UsageMission] {
    ...
    private func rankMissions(_ missions: [UsageMission], byItemsUsed allAtRiskItems: [PantryItem]) -> [UsageMission] {
        return missions.sorted { mission1, mission2 in
            ...
            if mission1.pantryItems.count != mission2.pantryItems.count {
                return mission1.pantryItems.count > mission2.pantryItems.count
            }
    ...
    private func calculateRescueScore(atRiskItems: [PantryItem], missions: [UsageMission]) -> Double {
        guard !atRiskItems.isEmpty else { return 0 }

        let itemsWithMissions = Set(missions.flatMap { $0.pantryItems.map { $0.id } })
    ...
    private func findMostUrgentTimeRemaining(from items: [PantryItem]) -> String {
    ...
    private func createFallbackMission(for item: PantryItem) -> UsageMission {
    ...
    private func determineUrgency(for item: PantryItem) -> UrgencyLevel {
        let calendar = Calendar.current
        let now = Date()
