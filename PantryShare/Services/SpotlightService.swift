...
enum SpotlightService {
    ...
    private static let domainIdentifier = "com.everwise.PantryShare.pantryItems"

    /// Index a single pantry item in Spotlight
    static func indexItem(_ item: PantryItem) {
    ...
    static func indexItems(_ items: [PantryItem]) {
    ...
    /// Remove all PantryShare items from Spotlight
    static func removeAllItems() {
        CSSearchableIndex.default().deleteSearchableItems(withDomainIdentifiers: [domainIdentifier]) { error in
            if let error {
                ...
                logger.info("Cleared all PantryShare items from Spotlight")
            }
        }
    }
    ...
    private static func buildDescription(for item: PantryItem) -> String {
        var parts: [String] = []
    ...
    private static func buildKeywords(for item: PantryItem) -> [String] {
