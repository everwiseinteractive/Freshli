...
final class FreshliService {
    ...
    func deleteItem(_ item: FreshliItem) {
    ...
    func markAsConsumed(_ item: FreshliItem) {
    ...
    func markAsShared(_ item: FreshliItem) {
        item.isShared = true
        notificationService?.cancelReminder(for: item)
        do {
            try modelContext.save()
    ...
    func markAsDonated(_ item: FreshliItem) {
        item.isDonated = true
        notificationService?.cancelReminder(for: item)
        do {
            try modelContext.save()
    ...
    func fetchActiveItems() -> [FreshliItem] {
    ...
    func fetchExpiringItems(withinDays days: Int = 3) -> [FreshliItem] {
    ...
    func fetchItems(category: FoodCategory? = nil, location: StorageLocation? = nil) -> [FreshliItem] {
    ...
    func updateItemExpiryDate(_ item: FreshliItem, newDate: Date) {
    ...
    func seedSampleDataIfNeeded() {
        ...
        do {
            let items = PreviewSampleData.shared.sampleFreshliItems
            for item in items {
                modelContext.insert(item)
            }
            try modelContext.save()
