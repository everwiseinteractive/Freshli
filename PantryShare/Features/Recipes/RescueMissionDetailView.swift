...
struct RescueMissionDetailView: View {
    ...
    private var metadataSection: some View {
        HStack(spacing: PSSpacing.md) {
            metadataChip(
                ...
                value: String(mission.pantryItems.count),
    ...
    private var itemsYouHaveSection: some View {
        VStack(alignment: .leading, spacing: PSSpacing.md) {
            PSSectionHeader(
                ...
                subtitle: "\(mission.pantryItems.count) item\(mission.pantryItems.count > 1 ? "s" : "")"
            )
            .screenPadding()

            VStack(spacing: PSSpacing.sm) {
                ForEach(Array(mission.pantryItems.enumerated()), id: \.element.id) { index, item in
                    HStack(spacing: PSSpacing.md) {
                    ...
                    if index < mission.pantryItems.count - 1 {
                        Divider()
                    }
                }
            }
    ...
    private func completeAndMarkConsumed() {
        let itemIds = Set(mission.pantryItems.map { $0.id })
        ...
        for item in mission.pantryItems {
